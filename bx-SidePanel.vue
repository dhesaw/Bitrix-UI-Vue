            ...
            <Transition>
                <div v-if="showVar===true" class="side-panel side-panel-containerVue" :style="{'width':props.width}">
                    ...
                    <div @click="closeSidePanel" class="side-panel-label" :style="{'background-color': props.color,'max-width': '215',}">
                        ...
                        <span class="side-panel-label-text" style="color: rgb(255, 255, 255);">{{ label }}</span>
                    </div>
                    <div class="side-panel-extra-labels"></div>
                </div>
                </div>
            </Transition>
        </div>
...
function closeSidePanel() {
  showVar.value=false;
  setTimeout(() =>model.value=false, 300);
}
...
<style scoped>
.v-enter-active {
  animation: right-load 0.3s;
}

.v-leave-active {
  animation: right-load 0.3s reverse;
}
...
/* /bitrix/js/main/sidepanel/css/sidepanel.min.css?170614835112285 */
/* Start:/bitrix/js/main/sidepanel/css/sidepanel.min.css?170614835112285*/
.side-panel-overlay {
    position: fixed;
    top: 0;
    right: 0;
...
.side-panel-show-shadow.side-panel-containerVue {
...
.side-panel-fullscreen .side-panel-containerVue {
...
.side-panel-toolbar.--muted .side-panel-toolbar-collapse-btn,.side-panel-toolbar.--muted .side-panel-toolbar-more-btn {
    visibility: hidden;
    transition: none;
    opacity: 0
}
