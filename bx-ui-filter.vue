<template>
	<div class="template-filter">
			...
			<div v-if="model" v-for="(selectedEl,key) in model">
				<div v-if="fields[key]" class="main-ui-filter-search-square main-ui-filter-search-square-preset main-ui-square">
					...
					<div class="main-ui-item-icon-block">
							<span class="main-ui-item-icon main-ui-search" @click="search"></span>
		...
		<BxTeleport :outerClick="true" ref="filter">
			<template #body="{popup}">
							<div class="main-ui-filter-wrapper main-ui-filter-theme-light main-ui-filter-with-additional-filters">
					<div class="main-ui-filter-inner-container">
						...
						<div class="main-ui-filter-field-container">
								<div v-for="(field,key) in fields" class="main-ui-filter-field-container-list">
									<component :is="field.component" v-model="filterData[key]" :key="key" v-bind="field.props">
										<template v-for="(slot,slotName) in slotsData[key]" v-slot:[slotName]="dataTemplate" >
											 <slot :name="key+'-'+slotName" v-bind="dataTemplate" />
										</template>
									</component>
								</div>
							</div>
						...
						<div class="main-ui-filter-bottom-controls">
							...
							<div class="main-ui-filter-field-preset-button-container">
								<div class="main-ui-filter-field-button-inner">
									<BxButton @click="search" label="Найти" type="search" />
									<BxButton @click="decline" label="Отменить" type="light" />
								</div>
							</div>
						</div>
					</div>
	...
	import { computed, DefineComponent, onMounted, reactive, ref, toRaw, useSlots, useTemplateRef, watch } from 'vue';
	import { ActionType, Fields, IElementPopup, IPosition, PopupLocation } from './globalInterface';
	import BxButton from './bx-button.vue';
	import BxUiCalendar from './bx-ui-calendar.vue';
	import BxUiInputNumber from './bx-ui-inputNumber.vue';
	...
	import BxUiInputSearch from './bx-ui-inputSearch.vue';
	
	const filterPopup=useTemplateRef('filter');
	const slots = useSlots();
	
	// onMounted(()=>getSlots('USE1R_ID'));
	
	const slotsData = computed(()=>{
		const templatesData=Object.entries(slots);
		// const needTemplate=templatesData.filter(el=>el[0].split('-')[0]==fieldName);
		// if (needTemplate==null || needTemplate==undefined) return [];
		
		let result = {};
		templatesData.forEach(elementTemplate => {
		// needTemplate.forEach(elementTemplate => {
			let slotName = elementTemplate[0].split('-');
			if (slotName.length>1){
				//@ts-ignore
				let fieldName:string = slotName.shift();
				if (result[fieldName]==null) result[fieldName]={};
				result[fieldName][slotName.join('-')]=elementTemplate[1];
			}
		});
	...
	const emit = defineEmits<{
		(e: 'action:search', data:any): void,
		(e: 'action:decline'): void,
		(e: 'delete:selectedElement', data:any): void
	}>();
	...
	const fields = computed(() => {
			...
			for(let key in props.fields){
					...
					switch(props.fields[key].type){
							case Fields.DATE:
									component=BxUiCalendar;
									break;
							case Fields.TEXT:
									component=BxUiInputText;
									break;
							case Fields.NUMBER:
									component=BxUiInputNumber;
									break;
							case Fields.DROPDOWN:
									component=BxUiSelect;
									break;
				case Fields.SEARCH:
					component=BxUiInputSearch;
				break;
					}
			...
			if (filterData.value[key] === undefined) {
				let value=null;
				if (model.value!=null && typeof model.value === 'object' && model.value[key] !== undefined) {
				...
				filterData.value[key] = value;
					...
					result[key]={
				label:props.fields[key].settings?.label ? props.fields[key].settings.label+": " : ': ',
							props:props.fields[key].settings,
							component:component,
					};
			}
			return result;
	...
	function deleteItem(key:string){
		if (model.value!==null && typeof model.value === 'object' && model.value[key]!==null) {
			delete model.value[key];
			filterData.value[key]=null;
			emit('delete:selectedElement',model.value);
		}
	}
	
	function openFilter(event: MouseEvent){
		filterData.value={};
		...
		let width=0;
		let previousElement=(event.currentTarget as HTMLElement).previousElementSibling;
		while(previousElement!=null){
			width+=previousElement.getBoundingClientRect().width;
			previousElement=previousElement.previousElementSibling;
		}
		filterPopup.value?.initPopup(event,PopupLocation.BOTTOM,{style:{'top':'9px', 'left':'-'+width+'px'}});
	}
	
	function prepareText(value:any, key:string){
		if (value===null) return '';
		if (value===true || value===false) return value?"Да":"Нет";
		if (Array.isArray(value)){
			return value.length+" элемента";
		}
	...
	function search(){
		...
		for(let key in filterData.value){
			if(filterData.value[key]!=null){
				...
				result[key]=filterData.value[key];
			}
		}
		model.value=result;
		filterData.value={};
		filterPopup.value?.close();
		emit('action:search',result);
	}
	...
	.filter-popup .left-menu-filter{
		height: 100%;
		display: flex;
		flex-direction: column;
		background: red;
	}
	.filter-popup .left-menu-filter .saved-filters{
		flex-grow: 1;
		height: 100%;
	}
	...
	.main-ui-item-icon-block .main-ui-item-icon.main-ui-search, .main-ui-item-icon-block .main-ui-item-icon.main-ui-delete {
			-webkit-transition: all 150ms;
			-o-transition: all 150ms;
			transition: all 150ms;
	}
