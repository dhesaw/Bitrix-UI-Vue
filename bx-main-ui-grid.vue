...
<div class="main-grid">
    <div class="main-grid-wrapper">
        ...
        <table class="main-grid-table" style="height: 1px" >
            <thead class="main-grid-header">
            <tr class="main-grid-row-head">
                <th
                v-if="selectedable"
                class="main-grid-cell-head main-grid-cell-static main-grid-cell-checkbox"
                >
                <span class="main-grid-cell-head-container">
                        <span class="main-grid-checkbox-container" @click="checkboxAllClick">
                            <input 
                              type="checkbox" 
                              :checked="checkboxSelected.length==optionsByPage[currentPage].length"
                              :indeterminate="checkboxSelected.length>0 && checkboxSelected.length!==optionsByPage[currentPage].length" 
            ...
            <tbody>
            <tr v-if="optionsByPage.length>0 && optionsByPage[currentPage].length > 0 && !loading" v-for="(row, index) in optionsByPage[currentPage]" class="main-grid-row main-grid-row-body" :class="{'main-grid-row-checked': checkboxSelected.includes(index)}">
                <td v-if="selectedable" class="main-grid-cell main-grid-cell-checkbox">
                    <span class="main-grid-cell-content" @click="checkBoxClick(index)">
                        <input type="checkbox" :checked="checkboxSelected.includes(index)" class="main-grid-row-checkbox main-grid-checkbox" />
                    </span>
                </td>
                <td class="main-grid-cell main-grid-cell-action">
                    <span v-if="action!==null" class="main-grid-cell-content">
                ...
                <div class="main-grid-cell-inner">
                    <span class="main-grid-cell-content">
                        <template v-if="$slots[`body-${col.propertyName}`]">
                            <slot v-bind="{ row, col, index}" :name="`body-${col.propertyName}`" >
                            </slot>
                        </template>
                        <template v-else>
            ...
            <tr v-else class="main-grid-row main-grid-row-empty main-grid-row-body">
              <td class="main-grid-cell main-grid-cell-center" :colspan="collumns.length+2">
                <div  class="main-grid-empty-block">
                  <div v-if="!loading" class="main-grid-empty-inner">
                    <div class="main-grid-empty-image"></div>
                    <div class="main-grid-empty-text">Нет данных</div>
                  </div>
                  <div style="height: 253px;" v-else>
    ...
    <div class="main-grid-bottom-panels">
        <div class="main-grid-nav-panel">
            <div class="main-grid-panel-wrap">
                <table v-if="!loading" class="main-grid-panel-table">
                    <tbody>
                        <tr>
                            <td class="main-grid-panel-total main-grid-panel-cell main-grid-cell-left">
                                <div class="main-grid-panel-content">
                                    <span class="main-grid-panel-content-title">Всего:</span>&nbsp;
                                    <span class="main-grid-panel-content-text">{{ options?.length || 0 }}</span>
                                </div>
                            </td>
                                ...
                                <div class="main-ui-pagination">
                                    <div class="main-ui-pagination-pages">
                                        ...
                                        <div class="main-ui-pagination-pages-list">
                                            <span v-for="(data, index) in optionsByPage">
                                              <span v-if="index === currentPage" class="main-ui-pagination-page main-ui-pagination-active">{{ index+1 }}</span>
                                              <a v-else @click="currentPage = index" class="main-ui-pagination-page">{{ index+1 }}</a>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="main-ui-pagination-arrows">
                                      <span class="main-ui-pagination-arrow main-ui-pagination-prev" 
                                        :class="{'disabled': currentPage === 0}" 
                                        @click="currentPage === 0 ? null : currentPage--"
                                      >
                                        Предыдущая
                                      </span>
                            ...
                            <td class="main-grid-panel-cell main-grid-panel-limit main-grid-cell-right">
                                <span class="main-grid-panel-content">
                                    <span class="main-grid-panel-content-title">На странице:</span>
                                    <span class="main-dropdown main-grid-popup-control main-grid-panel-select-pagesize" data-hasPopup="true" 
                                      @click="paginatorPopup.initPopup(
                                        $event, 
                                        ...
                                        <span class="main-dropdown-inner">{{ rowsPerPageOptions[selectedPerPageIndex] }}</span>
                                    </span>
                                </span>
                            </td>
                        </tr>
              ...
              <tbody>
                <tr class="main-grid-control-panel-row">
                  <td class="main-grid-control-panel-cell">
                    <span class="main-grid-panel-control-container">
                      <!-- <span class="main-grid-buttons icon remove">
                        Удалить
                      </span> -->
                    </span>
                  </td>
    ...
    <template #body="{popup}">
      <div class="menu-popup" style="display: block;"> 
        <div v-for="(paginatorItem,index) in rowsPerPageOptions" class="menu-popup-items">
          <span class="menu-popup-item" :class="{
            'main-dropdown-item-not-selected': index!==selectedPerPageIndex, 
            'main-dropdown-item-selected': index===selectedPerPageIndex
            }"
            @click="changePageIndex(index)"
          >
            <span class="menu-popup-item-icon"></span>
            <span class="menu-popup-item-text">
              <span class="main-dropdown-item">{{ paginatorItem }}</span>
            </span>
          </span>
        </div>
      </div>
  ...
  <BxTeleport ref="rowAction">
    <template #body="{popup}">
      <div class="menu-popup" style="display: block;">    
      <div v-for="(actionItem, index) in action" class="menu-popup-items" :class="{ 'disabled': (actionItem.disabled!=null && actionItem.disabled(popup.data)) }">
        <a class="menu-popup-item menu-popup-no-icon" @click="doActionHandler(actionItem, popup)">
          <span class="menu-popup-item-text">{{ actionItem.label }}</span>
        </a>
      </div>
    </div>
    </template>
...
const emit = defineEmits<{
    (e: 'action:open', data:IElementPopup): void,
    (e: 'action:closed', data:IElementPopup): void,
    (e: 'paginator:open', data:IElementPopup): void,
    (e: 'paginator:closed', data:IElementPopup): void,
    (e: 'initialize'): void
}>();
const selectedPerPageIndex=ref(props.defaultPageIndex);
const currentPage=ref(0);
const paginatorPopup=useTemplateRef('paginator');
const rowActionPopup=useTemplateRef('rowAction');
...
function resizeHeader(event: MouseEvent, target: HTMLElement) {
  target.style.width = `${event.clientX - target.getBoundingClientRect().left}px`;
}

function startMove(event:any) {
    const fnInstanse = (eventThis:any) => resizeHeader(eventThis, event.target?.closest(".main-grid-cell-head.main-grid-cell-left"));
    event.target?.closest(".main-grid-table").addEventListener("mousemove", fnInstanse);
    event.target?.closest(".main-grid-table").addEventListener("mouseup",()=>
      event.target?.closest(".main-grid-table").removeEventListener("mousemove",fnInstanse)
    );
}

function checkboxAllClick() {
  if (checkboxSelected.value.length >0) {
    checkboxSelected.value = [];
  } else {
    checkboxSelected.value = optionsByPage.value[currentPage.value].map((item, index) => index);
  }
}

function checkBoxClick(index: number) {
  console.log(index);
  if (checkboxSelected.value.includes(index)) {
    checkboxSelected.value = checkboxSelected.value.filter((item) => item !== index);
  } else {
    checkboxSelected.value.push(index);
  }
}

function doActionHandler(action: IAction, popup: any) {
    if (popup?.data) {
      if (action.disabled && action.disabled(popup.data)) return;
        action.handlerFn(popup.data);
        rowActionPopup.value?.close();   
    }
}
function changePageIndex(index: number) {
  currentPage.value=0;
  selectedPerPageIndex.value = index;
  rowActionPopup.value?.close();
  paginatorPopup.value?.close();
}
...
watch(()=> selectedPerPageIndex.value, () => {
  rowActionPopup.value?.close();
  paginatorPopup.value?.close();
  checkboxSelected.value=[];
});
...
.main-ui-pagination-arrow.disabled{
  color: #cccdcf;
}

.main-ui-pagination-arrow:not(.disabled):hover{
  cursor: pointer;
  color:#2067b0;
}
...
.menu-popup .menu-popup-items.disabled{
    background-color: #f6f8f9;
}

.menu-popup .menu-popup-items.disabled .menu-popup-item-text{
    color: #a7a7a7;
    cursor: not-allowed;
}
...
.main-grid-panel-custom {
  max-width: 300px;
}

.main-grid-panel-custom select {
  max-width: 220px;
}
...
.main-grid-editor-textarea:focus {
  border-color: #7d7d7d;
}
...
.main-dropdown {
  ...
  border: 1px solid #c6cdd3;
  border-radius: var(
    --main-dropdown-border-radius,
    var(--ui-border-radius-2xs, 2px)
  );
  background: #fff;
  color: #5c6470;
  text-align: left;
...
.main-dropdown-item-selected .menu-popup-item-icon {
  background: url(data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%2212%22%20height%3D%229%22%3E%3Cpath%20fill%3D%22%232066B0%22%20d%3D%22M1.36%203.446L4.013%206.1%2010.316%200l1.36%201.36-6.299%206.1-1.364%201.36-1.36-1.36L0%204.806z%22/%3E%3C/svg%3E)
    no-repeat center;
}
...
.main-grid-interface-settings-icon {
  ...
  width: 16px;
  height: 17px;
  background: url(data:image/svg+xml;base64,) 50% 50% no-repeat;
...
.main-grid-cell-action
  + .main-grid-cell-head:not([class*="main-grid-column-color"])
  .main-grid-cell-head-container {
  padding-left: 2px;
}

.main-grid-cell-action
  + .main-grid-cell:not([class*="main-grid-column-color"])
  .main-grid-cell-content {
  margin-left: 3px;
}

.main-grid-interface-settings-icon.main-grid-interface-settings-icon-active {
  opacity: 1;
}
...
.main-grid-fade-shadow-left {
  ...
  overflow: hidden;
  width: 10px;
  border-left: 1px rgba(238, 242, 244, 0.5) solid;
  content: "";
  opacity: 1;
...
.main-grid-fade-right .main-grid-fade-shadow-right {
  opacity: 1;
}

.main-grid-fade-left .main-grid-fade-shadow-left::before {
  opacity: 1;
}
...
.main-grid-cell-head {
  ...
  vertical-align: middle;
  cursor: pointer;
  transition: background-color 0.3s cubic-bezier(0, 0.465, 0.75, 0.75);
  white-space: nowrap;
  background-color: #fff;
...
.main-grid-cell-head:last-child {
  border-right: 0;
}

.main-grid-row-head .main-grid-cell-head.main-grid-cell-last {
...
.main-grid-border .main-grid-row-head .main-grid-cell-head:first-child {
...
.main-grid-col-no-sortable {
  cursor: default;
}

.main-grid-compact .main-grid-cell-head-container {
  padding: 9px 10px;
}

.main-grid-compact .main-grid-head-title {
  font-size: 13px;
}
...
.main-grid-compact .main-grid-cell-sortable-down {
  padding-right: 21px;
}

.main-grid-cell-sortable {
...
.main-grid-cell-sortable-down:hover:before {
  opacity: 0.6;
}
...
.main-grid:not(.main-grid-ondrag)
  .main-grid-row.main-grid-row-over
  > .main-grid-cell {
  background-color: #f6f8f9;
}
...
.main-grid:not(.main-grid-ondrag)
  .main-grid-row-checked:hover
  > .main-grid-cell,
.main-grid:not(.main-grid-ondrag)
  .main-grid-row.main-grid-row-checked.main-grid-row-over
  > .main-grid-cell,
.main-grid-row-checked:hover > .main-grid-cell {
  background-color: #eff7d7;
}

.main-grid-row .main-grid-cell:last-child {
  border-right: none;
}
...
.main-grid-border .main-grid-row .main-grid-cell:first-child {
...
.main-grid-row
  .main-grid-cell-content-action-pin.main-grid-cell-content-action-active::before {
...
.main-grid-row:hover
  .main-grid-cell-content-action-pin.main-grid-cell-content-action-active::before,
.main-grid-row:hover
  .main-grid-cell-content-action-mute.main-grid-cell-content-action-active::before {
  opacity: 0.8;
}

.main-grid-row:hover
  .main-grid-cell-content-action-pin.main-grid-cell-content-action-active:hover::before,
.main-grid-row:hover
  .main-grid-cell-content-action-mute.main-grid-cell-content-action-active:hover::before {
  opacity: 1;
}
...
.main-grid-compact .main-grid-cell-content {
...
.main-grid-cell-text-limited-btn:hover {
...
.main-grid-content-note {
  ...
  width: 13px;
  height: 11px;
  background: url(data:image/svg+xml;base64,) 0 -187px no-repeat;
  opacity: 0.4;
  cursor: pointer;
  transition: opacity 200ms ease;
}

.main-grid-content-note:hover {
  opacity: 0.7;
}
...
.main-grid-cell-foot .main-grid-cell-content {
  padding: 10px 13px 22px;
}

.main-grid-compact .main-grid-cell-foot .main-grid-cell-content {
...
.main-grid-row-action-button:hover {
  opacity: 0.7;
}
...
textarea.main-grid-cell-content-edit {
  height: auto;
}

input[type="file"].main-grid-cell-content-edit {
  display: none;
}
...
.main-grid-control-panel-table tr {
  height: 42px;
}
...
.main-grid-panel-counter.main-grid-panel-counter-for-all-enable
  .main-grid-panel-counter-for-selected {
  display: none;
}

.main-grid-panel-counter.main-grid-panel-counter-for-all-enable
  .main-grid-panel-counter-for-all {
  display: block;
}
...
.main-grid-cell-center {
  text-align: center;
}

.main-grid-cell-right {
  text-align: right;
}

.main-grid-cell-justify {
  text-align: justify;
}
...
.main-grid-control-panel-cell.main-grid-control-panel-cell-checkbox {
  padding-left: 0;
}
...
.main-grid-control-panel-content {
  white-space: nowrap;
}
...
.main-grid-control-panel-action-edit {
  background-position: 0 0;
}

.main-grid-control-panel-action-remove {
  background-position: 0 -17px;
}

.main-grid-control-panel-action-revert {
  background-position: 0 -34px;
}

.main-grid-control-panel-action-icon:hover {
  opacity: 1;
}
...
html:not(.bx-firefox)
  .main-grid-cell-head:nth-last-of-type(2)
  .main-grid-resize-button::after {
  right: 1px;
}
...
.main-grid-cell-head-drag .main-grid-resize-button {
  right: 0;
}
...
.main-grid-border .main-grid-cell-head-drag .main-grid-cell-head-container {
  top: 1px;
}
...
.main-grid-cell-head-ondrag.main-grid-cell-last .main-grid-cell-head-container {
  border-right-color: transparent;
}
...
.main-grid-more-btn:active {
  background: #f5f5f5;
  box-shadow: inset 0 -4px 3px -5px rgba(0, 0, 0, 0.7);
}
...
.main-table-more-btn:hover .main-grid-more-icon {
  opacity: 1;
}
...
.main-grid-control-sort {
  ...
  width: 14px;
  height: 14px;
  background: url(data:image/svg+xml;base64,) 3px -290px no-repeat;
}

.main-grid-control-sort-desc,
.main-grid-cell-head:hover .main-grid-control-sort-hover-desc,
.main-grid-cell-head:active .main-grid-control-sort-hover-desc {
  visibility: visible;
}
...
.main-grid-cell-head:active .main-grid-control-sort-hover-asc {
  visibility: visible;
  background: url(data:image/svg+xml;base64,) 3px -291px no-repeat;
  transform: rotate(180deg);
}

.main-grid-col-no-sortable .main-grid-control-sort-asc,
.main-grid-col-no-sortable.main-grid-cell-head:hover
  .main-grid-control-sort-hover-asc,
.main-grid-col-no-sortable.main-grid-cell-head:active
  .main-grid-control-sort-hover-asc,
.main-grid-col-no-sortable .main-grid-control-sort-desc,
.main-grid-col-no-sortable.main-grid-cell-head:hover
  .main-grid-control-sort-hover-desc,
.main-grid-col-no-sortable.main-grid-cell-head:active
  .main-grid-control-sort-hover-desc {
  visibility: hidden;
}
...
.main-grid-head-checkbox-container label.main-grid-checkbox {
  display: none;
}
...
.menu-popup-item.icon .menu-popup-item-icon {
  background: url(data:image/svg+xml;base64,) 999px 999px no-repeat;
}

.menu-popup-item.icon.edit .menu-popup-item-icon {
...
.menu-popup-item.icon.remove .menu-popup-item-icon {
  background-position: 50% -277px;
}
...
[id*="popup-window-content-menu-popup-main-grid-actions-menu"]
  .menu-popup-item-text,
[id*="menu-popup-main_grid_submenu"] .menu-popup-item-text {
  padding-right: 22px;
}
...
.main-grid-row-custom:hover > .main-grid-cell {
...
.main-grid-cell-head-container
  .main-grid-checkbox.main-grid-row-checkbox.main-grid-check-all {
  width: 14px;
  height: 14px;
}
...
.main-grid-action-panel.main-grid-fixed-bottom {
  z-index: 80;
}
...
.main-grid-action-panel.main-grid-fixed-bottom .main-grid-panel-counter {
  opacity: 1;
}
...
.main-grid-buttons {
  ...
  border: 1px solid #c6cdd3;
  border-radius: var(
    --main-grid-buttons-border-radius,
    var(--ui-border-radius-2xs, 2px)
  );
  background: none;
  color: #535c69;
...
.main-grid-action-panel:not(.main-grid-disable) .main-grid-buttons.save:hover {
  background: #d2f95f;
}
...
.main-grid-buttons.cancel:hover {
  background: #cc1a00;
}
...
.main-grid-buttons.icon.edit::before {
  left: 12px;
}
...
.main-grid-control-panel-cell:first-child {
  padding-left: 6px;
}

.grid-mode .workarea-content-paddings,
.grid-mode #workarea-content {
  padding: 0;
}

.main-dropdown-item[data-pseudo="true"] {
  opacity: 0.6;
}
...
.popup-window[id*="-grid-settings-window"]
...
.main-grid-settings-window-list-label {
  padding-bottom: 5px;
}
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-label:not([contenteditable="true"]) {
  transition: background 200ms;
}
...
.main-grid-settings-window-list-item-label[contenteditable="true"] {
  border: 1px rgba(59, 153, 252, 0.6) solid;
}

.main-grid-settings-window-list-item-checkbox:checked
  + .main-grid-settings-window-list-item-label,
.main-grid-settings-window-list-item:hover
  .main-grid-settings-window-list-item-label:not([contenteditable="true"]) {
  background: #b3eafc;
}

.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-edit-button {
  position: absolute;
  top: 0;
  right: 24px;
  z-index: 2;
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-edit-button::before {
  position: relative;
  top: 2px;
  left: -5px;
  z-index: 1;
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-edit-button::after {
  position: absolute;
  top: 5px;
  left: -10px;
  display: inline-block;
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-edit-button:hover,
.main-grid-settings-window-list-item-label[contenteditable="true"]
  + .main-grid-settings-window-list-item-edit-button {
  opacity: 1;
}
...
.main-grid-settings-window-list-item-label[contenteditable="true"]
  + .main-grid-settings-window-list-item-edit-button:hover::after {
  background: #d2f95f;
}
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-sticky-button {
  position: absolute;
  top: 5px;
  right: 0;
  width: 24px;
...
.main-grid-settings-window-list-item
  .main-grid-settings-window-list-item-sticky-button::after {
  display: block;
  width: 24px;
  height: 24px;
...
.main-grid-settings-window-list-item.main-grid-settings-window-list-item-sticked
  .main-grid-settings-window-list-item-sticky-button {
  opacity: 1;
}
...
.main-grid-settings-window-list-item-checkbox:not(:checked)
  ~ .main-grid-settings-window-list-item-sticky-button {
  display: none;
}
...
.main-grid-settings-window-list-item-edit
  .main-grid-settings-window-list-item-edit-input {
  z-index: 1;
  display: block;
}
...
.main-grid-settings-window-actions-item:hover {
  color: #000;
}

.main-grid-settings-window-actions-item-close {
  padding-left: 13px;
}
...
.main-grid-settings-window-actions-item-close:hover::before {
  opacity: 1;
}

.main-grid-settings-window-list-item-checked
  .main-grid-settings-window-list-item-label::before {
...
.main-grid-settings-window-actions-item-reset::before {
  ...
  width: 10px;
  height: 10px;
  background: no-repeat 0 -22px url(data:image/svg+xml;base64,);
  background-size: 100%;
  content: "";
  transition: all 200ms;
}

.main-grid-settings-window-actions-item-reset:hover {
  opacity: 1;
}
...
.main-grid-settings-window-for-all
  .main-grid-settings-window-for-all-checkbox:hover,
.main-grid-settings-window-for-all
  .main-grid-settings-window-for-all-label:hover {
  opacity: 0.6;
}

.main-grid-settings-window-for-all
  .main-grid-settings-window-for-all-checkbox:checked,
.main-grid-settings-window-for-all
  .main-grid-settings-window-for-all-checkbox:checked
  + .main-grid-settings-window-for-all-label {
  opacity: 1;
}
...
.main-adm-panel-mini-grid-state .main-grid-empty-block {
  height: calc(100vh - 329px);
}

.main-adm-panel-full-grid-state .main-grid-empty-block {
...
.main-grid-row-custom .main-grid-plus-button + .main-grid-cell-content {
  margin-left: 34px;
}

.main-grid-row-custom
  .main-grid-minus-button
  + .main-grid-cell-content
  .tasks-grid-wrapper,
.main-grid-row-custom
...
.main-grid-row-expand .main-grid-plus-button {
  opacity: 0.7;
}

.main-grid-row-expand .main-grid-plus-button:hover {
  opacity: 1;
}
...
.main-grid-plus-button:before {
  ...
  width: 22px;
  height: 14px;
  background: url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%2214%22%20height%3D%2214%22%20viewBox%3D%220%200%2014%2014%22%3E%0A%20%20%3Cg%20fill%3D%22none%22%20fill-rule%3D%22evenodd%22%3E%0A%20%20%20%20%3Ccircle%20cx%3D%227%22%20cy%3D%227%22%20r%3D%226.5%22%20stroke%3D%22%23525C69%22%20opacity%3D%22.234%22/%3E%0A%20%20%20%20%3Cpath%20fill%3D%22%2380868E%22%20d%3D%22M8%2C6%20L8%2C4%20L6%2C4%20L6%2C6%20L4%2C6%20L4%2C8.01443104%20L6%2C8.01443104%20L6%2C10%20L8%2C10%20L8%2C8.01443104%20L10%2C8.01443104%20L10%2C6%20L8%2C6%20Z%22%20opacity%3D%22.7%22/%3E%0A%20%20%3C/g%3E%0A%3C/svg%3E%0A")
...
.main-grid-minus-button:hover:before {
  opacity: 1;
}
...
.main-grid-settings-window-select-link {
  margin-right: 20px;
  border-bottom: 1px rgba(128, 134, 142, 0.35) dashed;
  color: #80868e;
  opacity: 0.8;
  cursor: pointer;
  transition: opacity 200ms;
...
.main-grid-settings-window-select-link:hover {
  opacity: 1;
}

.main-grid-settings-window-select-link:active {
  opacity: 0.6;
}
...
.main-grid-table-fake .main-grid-cell-content {
  opacity: 1;
}
...
.main-grid-error.main-grid-error-show {
  opacity: 1;
}
...
.main-grid-message:last-child {
  margin-bottom: 0;
}

.main-grid-message-title {
...
.main-grid-show-loader {
  animation: showLoader 0.5s linear forwards;
}

.main-grid-hide-loader {
  animation: hideLoader 0.5s linear forwards;
}
...
.main-grid-cell-head
  .main-grid-cell-counter-left-aligned
  + .main-grid-cell-head-container {
  padding-left: 0;
  margin-left: 7px;
...
.main-grid-label-remove-inside {
  background: url("data:image/svg+xml;charset=utf-8,%3Csvg width='8' height='9' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1.316.685l2.64 2.641L6.598.686l1.174 1.173L5.13 4.5l2.642 2.64-1.174 1.175-2.642-2.641-2.64 2.64L.142 7.142 2.782 4.5.142 1.86z' fill='%23525C69' fill-rule='evenodd'/%3E%3C/svg%3E")
    center center no-repeat;
  width: 16px;
  height: 16px;
  margin-right: -5px;
...
.main-grid-tag-add {
  background: url("data:image/svg+xml;charset=utf-8,%3Csvg width='9' height='9' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M5.326 9V5.318H9V3.75H5.326V0h-1.63v3.75H0v1.568h3.696V9z' fill='%23525C69' fill-rule='nonzero'/%3E%3C/svg%3E")
    center center no-repeat;
  display: inline-flex;
  width: 31px;
  height: 21px;
  ...
  margin-top: 3px;
  margin-bottom: 3px;
  border: 1px rgba(82, 92, 105, 0.37) solid;
  cursor: pointer;
  transition: 200ms all ease;
  opacity: 0;
}
